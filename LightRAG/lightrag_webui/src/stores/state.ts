...
const initAuthState = (): { isAuthenticated: boolean; isGuestMode: boolean; coreVersion: string | null; apiVersion: string | null; username: string | null; webuiTitle: string | null; webuiDescription: string | null; lastTokenRenewal: string | null; tokenExpiresAt: number | null } => {
  const token = scopedStorage.getItem('LIGHTRAG-API-TOKEN');
  const coreVersion = scopedStorage.getItem('LIGHTRAG-CORE-VERSION');
  const apiVersion = scopedStorage.getItem('LIGHTRAG-API-VERSION');
  const webuiTitle = scopedStorage.getItem('LIGHTRAG-WEBUI-TITLE');
  const webuiDescription = scopedStorage.getItem('LIGHTRAG-WEBUI-DESCRIPTION');
  const lastTokenRenewal = scopedStorage.getItem('LIGHTRAG-LAST-TOKEN-RENEWAL');
...
export const useAuthStore = create<AuthState>(set => {
  // Get initial state from scopedStorage
  ...
  return {
    ...
    login: (token, isGuest = false, coreVersion = null, apiVersion = null, webuiTitle = null, webuiDescription = null) => {
      scopedStorage.setItem('LIGHTRAG-API-TOKEN', token);

      if (coreVersion) {
        scopedStorage.setItem('LIGHTRAG-CORE-VERSION', coreVersion);
      }
      if (apiVersion) {
        scopedStorage.setItem('LIGHTRAG-API-VERSION', apiVersion);
      }

      if (webuiTitle) {
        scopedStorage.setItem('LIGHTRAG-WEBUI-TITLE', webuiTitle);
      } else {
        scopedStorage.removeItem('LIGHTRAG-WEBUI-TITLE');
      }

      if (webuiDescription) {
        scopedStorage.setItem('LIGHTRAG-WEBUI-DESCRIPTION', webuiDescription);
      } else {
        scopedStorage.removeItem('LIGHTRAG-WEBUI-DESCRIPTION');
      }
      ...
      scopedStorage.setItem('LIGHTRAG-LAST-TOKEN-RENEWAL', formattedTime);
    ...
    logout: () => {
      ...
      scopedStorage.removeItem('LIGHTRAG-LAST-TOKEN-RENEWAL');
    ...
    setTokenRenewal: (renewalTime, expiresAt) => {
      ...
      // Update scopedStorage with human-readable format
      scopedStorage.setItem('LIGHTRAG-LAST-TOKEN-RENEWAL', formattedTime);
