...
interface SettingsState {
  ...
  // Multi-workspace selection (LIGHTRAG-WORKSPACE header). Empty string =
  // server default workspace. Only meaningful when the server runs in
  // multi-workspace mode; the selector is hidden otherwise.
...
const useSettingsStoreBase = create<SettingsState>()(
  persist(
    (set) => ({
      ...
      setActiveWorkspace: (activeWorkspace: string) => set({ activeWorkspace }),
    ...
    }),
    {
      ...
      migrate: (state: any, version: number) => {
        ...
        if (version < 21) {
          // Add activeWorkspace field for older versions (multi-workspace mode)
