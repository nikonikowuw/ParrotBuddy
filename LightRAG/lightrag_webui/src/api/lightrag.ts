  ...
  refreshTokenPromise = (async () => {
    try {
      ...
      if (response.data.access_token && !response.data.auth_configured) {
        ...
        scopedStorage.setItem('LIGHTRAG-API-TOKEN', newToken);
        // Update auth state
...
axiosInstance.interceptors.request.use((config) => {
  ...
  // Multi-workspace selection: route the request to the chosen workspace.
  // Empty string = server default (no header sent).
  const activeWorkspace = useSettingsStore.getState().activeWorkspace
  if (activeWorkspace) {
    config.headers['LIGHTRAG-WORKSPACE'] = activeWorkspace
  }
  return config
})
  ...
  async (error: AxiosError) => {
    if (error.response) {
      if (error.response?.status === 401) {
        ...
        const authStore = useAuthStore.getState();
        const currentToken = scopedStorage.getItem('LIGHTRAG-API-TOKEN');
