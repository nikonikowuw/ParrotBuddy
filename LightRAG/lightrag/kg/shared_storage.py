...
# Set once by the first LightRAG.initialize_storages() as the process-wide
# fallback for namespace calls that omit workspace.
_default_workspace: Optional[str] = None

# Per-context workspace (request-scoped in the multi-workspace API server).
# When set (via set_current_workspace at the request boundary) it takes
# precedence over _default_workspace, so multiple LightRAG instances with
# different workspaces in one process don't leak implicit namespace calls
# into the first instance's workspace. get_default_workspace() resolves
# ContextVar -> _default_workspace -> None.
_current_workspace_var: ContextVar[Optional[str]] = ContextVar(
...
def get_final_namespace(namespace: str, workspace: str | None = None):
    if workspace is None:
        # Falls back to the per-context workspace (ContextVar) when set,
        # otherwise to the process-wide _default_workspace global.
...
def get_default_workspace() -> Optional[str]:
    """
    Resolve the default workspace for namespace operations.

    Resolution order:
    1. Per-context workspace (ContextVar), set at the request boundary via
       set_current_workspace() in multi-workspace mode. Takes precedence so
       multiple LightRAG instances with different workspaces in one process
       don't leak implicit namespace calls into the first instance's workspace.
    2. Process-wide ``_default_workspace`` global, set once by the first
       LightRAG.initialize_storages() (backward-compatible behavior).

    Returns:
        The workspace string (empty string means global namespace) or None
        if neither is set.
    """
    current = _current_workspace_var.get()
...
def set_current_workspace(workspace: str | None):
    """
    Set the per-context workspace for the current async/thread context.

    Intended for request-scoped scoping in the multi-workspace API server:
    a request resolved to workspace B sets this (via the get_rag_for_request
    dependency) so any implicit get_namespace_data/lock call during that
    request lands on B instead of the process-wide default.

    Returns a reset token; pass it to reset_current_workspace() at request
    end. The ContextVar only affects the current context (and any child
    tasks spawned after the set), so it does not bleed across requests.
    """
    return _current_workspace_var.set(workspace)


def reset_current_workspace(token) -> None:
    """
    Reset the per-context workspace to its prior value.

    Use the token returned by set_current_workspace(). Must be called from
    the same context that set it (e.g. the request dependency's exit path).
    """
...
def get_pipeline_status_lock(
    ...
    """
    actual_workspace = workspace if workspace else get_default_workspace()
