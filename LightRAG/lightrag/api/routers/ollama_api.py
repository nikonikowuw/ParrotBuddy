...
class OllamaAPI:
    def __init__(self, rag: LightRAG, top_k: int = 60, api_key: Optional[str] = None):
        # `rag` may be a real LightRAG (unit tests) or a RequestRagProxy
        # (server multi-workspace mode). ollama_server_infos is resolved
        # lazily via the property below so the proxy is never touched at
        # construction time (no request context yet).
        self.rag = rag
        self.top_k = top_k
    ...
    @property
    def ollama_server_infos(self):
        return self.rag.ollama_server_infos
