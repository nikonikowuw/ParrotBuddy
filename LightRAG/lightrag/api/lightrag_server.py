...
from lightrag.api.workspace_registry import (
    RequestDocManagerProxy,
    RequestRagProxy,
    WorkspaceRegistry,
    conflicting_workspace_envs,
...
def create_app(args):
    ...
    async def lifespan(app: FastAPI):
        ...
        try:
            # Pre-warm the default workspace: build its LightRAG, run
            # initialize_storages() (which also sets the process-wide default
            # workspace and initializes pipeline_status) and migrate data.
            # Other workspaces are lazily built on first request.
            ...
            # Clean up database connections for every cached workspace instance
    ...
    # Per-workspace LightRAG factory. The captured LLM/embedding/storage/role
    # configuration is shared across workspaces; only ``workspace`` differs.
    # The registry lazily builds and caches one instance per workspace.
    ...
    # Multi-workspace registry. When LIGHTRAG_MULTI_WORKSPACE is off, only the
    # default workspace (args.workspace) is served, matching the legacy
    # single-instance behavior. The LIGHTRAG-WORKSPACE request header selects
    # the workspace per request (see lightrag.api.workspace_registry).
    multi_workspace = (
        get_env_value("LIGHTRAG_MULTI_WORKSPACE", "false").lower() == "true"
    )
    workspaces_env = get_env_value("WORKSPACES", "")
    allowlist = None
    if multi_workspace and workspaces_env:
        allowlist = {w.strip() for w in workspaces_env.split(",") if w.strip()}

    if multi_workspace:
        conflicts = conflicting_workspace_envs()
        if conflicts:
            logger.warning(
                "Multi-workspace mode is on but per-backend workspace env vars are set ("
                f"{', '.join(conflicts)}). These force a single workspace process-wide "
                "and would defeat per-request isolation; unset them to use "
                "LIGHTRAG-WORKSPACE header-based selection."
            )

    registry = WorkspaceRegistry(
        rag_factory=build_rag_for_workspace,
        doc_manager_factory=lambda ws: DocumentManager(
            args.input_dir, workspace=ws
        ),
    ...
    # Eagerly build + register the default workspace's LightRAG so it exists
    # before lifespan (matches the legacy single-instance build timing) and
    # so the request proxy / middleware can return it sync-ly from cache
    # without awaiting initialize_storages on it during request handling
    # (storage init runs in lifespan via registry.initialize_default()).
    default_rag = build_rag_for_workspace(args.workspace)
    registry.register_default(default_rag)

    # Install the middleware that resolves the per-request LightRAG /
    # DocumentManager from the LIGHTRAG-WORKSPACE header and stashes them in
    # ContextVars for the route-handler proxies (RequestRagProxy /
    # RequestDocManagerProxy) to pick up.
    setup_workspace_middleware(app)

    # Add routes. The factories close over proxies; the middleware swaps the
    # target instance per request based on the LIGHTRAG-WORKSPACE header.
    # root_path is set on the app for reverse proxy support; routes stay at
    # their natural paths and are prefixed by the proxy or uvicorn --root-path.
    app.include_router(
        create_document_routes(RequestRagProxy(), RequestDocManagerProxy(), api_key)
    )
    app.include_router(create_query_routes(RequestRagProxy(), api_key, args.top_k))
    app.include_router(create_graph_routes(RequestRagProxy(), api_key))

    # Add Ollama API routes (rag is a proxy; ollama_server_infos is resolved
    # lazily via OllamaAPI.ollama_server_infos property at request time).
    ollama_api = OllamaAPI(RequestRagProxy(), top_k=args.top_k, api_key=api_key)
    ...
    async def get_status(request: Request, authenticated: bool = Depends(auth_status)):
        ...
        try:
            ...
            # /health reports the default workspace's configuration (always
            # pre-warmed at startup). pipeline_status above is already scoped
            # to the requested LIGHTRAG-WORKSPACE header.
            default_rag = registry.get_rag_if_cached(registry.default_workspace)
            ...
            status_data.update(
                {
                    ...
                    "configuration": {
                        ...
                        "vlm_process_enable": args.vlm_process_enable,
                        "workspace": default_workspace,
                        "storage_workspaces": _get_storage_workspaces(default_rag),
                        "max_graph_nodes": args.max_graph_nodes,
                        ...
                        "embedding_timeout": args.embedding_timeout,
                        "role_llm_config": default_rag.get_llm_role_config(),
                    ...
                    "llm_queue_status": await default_rag.get_llm_queue_status(
                        include_base=True
                    ),
                    "embedding_queue_status": await default_rag.get_embedding_queue_status(),
                    "rerank_queue_status": await default_rag.get_rerank_queue_status(),
                }
            )
            return status_data
        except Exception as e:
    ...
    @app.get(
        ...
        description=(
            "Returns the workspaces this server can serve. Used by the WebUI "
            "workspace selector. In single-workspace mode (the default) only "
            "the default workspace is returned; the WebUI hides the selector."
        ),
    )
    async def list_workspaces():
        """List workspaces available for per-request selection."""
        return {
            "multi_workspace": registry.multi_workspace,
            "default": registry.default_workspace,
            # Allowlist (WORKSPACES env) + any already-initialized workspaces
            # (e.g. warmed up by prior requests), so the selector can surface
            # workspaces that exist in storage even if not in the allowlist.
            "workspaces": sorted(
                set(registry.list_allowed_workspaces())
                | set(registry.list_initialized_workspaces())
            ),
        }
