...
def _file_save_error_token(status: int) -> str:
    """Map a WebUIFilePreviewError status to a stable UI-localizable token."""
    return {
        400: "invalid_path",
        403: "forbidden",
        404: "not_found",
...
class WebSocketChannel(BaseChannel):
    ...
    async def _dispatch_envelope(
        ...
        if t == "set_workspace_scope":
            ...
            return
        if t == "set_lightrag_workspaces":
            cid = envelope.get("chat_id")
            if not _is_valid_chat_id(cid):
                await self._send_event(connection, "error", detail="invalid chat_id")
                return
            workspaces = normalize_lightrag_workspaces(
                envelope.get("lightrag_workspaces")
            )
            # KB selection is not a security boundary (it only picks which
            # allowlisted workspace the lightrag_query tool fans out to, and the
            # tool re-validates every name server-side), so unlike
            # set_workspace_scope it needs no is_allowed / non-escalation gate
            # and may change mid-turn (it affects the next turn; the running
            # turn's metadata is snapshotted). Mirrors how other per-turn
            # metadata (cli_apps, mcp_presets) is handled.
            self._workspaces.persist_lightrag_workspaces(cid, workspaces)
            await self._send_event(
                connection,
                "session_updated",
                chat_id=cid,
                scope="metadata",
                lightrag_workspaces=workspaces,
            )
            return
    ...
    # -- File save (WebUI editor round-trip) ------------------------------

    async def _save_file_event(
        self,
        envelope: dict[str, Any],
    ) -> tuple[str, dict[str, Any]]:
        """Validate and persist an edited file from the WebUI editor.

        Envelope shape: ``{"type": "save_file", "chat_id": str,
        "request_id": str, "path": str, "content_b64": str}``.

        The path is resolved against the *persisted* workspace scope for the
        chat (never an envelope-supplied scope) so a remote client cannot
        escalate access by attaching a workspace_scope field. Writes go
        through :func:`write_file_bytes` which enforces the same boundary as
        the preview routes and only allows overwriting existing files.

        Returns ``("file_saved", payload)`` or ``("file_save_error",
        payload)`` where payload ``detail`` is a short stable token suitable
        for UI localization.
        """
        request_id = envelope.get("request_id")
        if not isinstance(request_id, str) or not request_id:
            return "file_save_error", {"detail": "missing_request_id"}
        cid = envelope.get("chat_id")
        if not _is_valid_chat_id(cid):
            return "file_save_error", {"request_id": request_id, "detail": "invalid_chat_id"}
        path = envelope.get("path")
        if not isinstance(path, str) or not path.strip():
            return "file_save_error", {"request_id": request_id, "detail": "missing_path"}
        content_b64 = envelope.get("content_b64")
        if not isinstance(content_b64, str) or not content_b64:
            return "file_save_error", {"request_id": request_id, "detail": "missing_content"}
        # Reject oversized payloads before decoding: a valid base64 frame for
        # MAX_FILE_SAVE_BYTES is at most ~(max/3)*4 chars, so anything longer
        # can only fail the write size check — skip the wasteful decode.
        if len(content_b64) > ((MAX_FILE_SAVE_BYTES + 2) // 3) * 4:
            return "file_save_error", {"request_id": request_id, "detail": "size"}
        scope = self._workspaces.scope_for_session_key(f"websocket:{cid}")

        def _decode_and_write() -> str:
            # Decode and write together so the up-to-16 MB base64 decode does
            # not block the event loop; only the small length pre-check above
            # runs inline.
            content = base64.b64decode(content_b64, validate=True)
            return write_file_bytes(path, content, scope=scope)

        try:
            saved = await asyncio.to_thread(_decode_and_write)
        except WebUIFilePreviewError as exc:
            # Must precede the ValueError catch: WebUIFilePreviewError is a
            # ValueError subclass raised by the write path.
            return (
                "file_save_error",
                {"request_id": request_id, "detail": _file_save_error_token(exc.status)},
            )
        except (ValueError, binascii.Error):
            return "file_save_error", {"request_id": request_id, "detail": "decode"}
        except Exception as exc:
        ...
        if msg.media:
            ...
            for entry in msg.media:
                ...
                if signed is not None:
                    urls.append(signed)
            if urls:
                # Keep ``path`` only for files the per-chat workspace scope
                # will actually serve in the preview panel; otherwise the
                # client would offer a preview that 403s and, worse, drop
                # the working download link for out-of-scope media.
                strip_out_of_scope_paths(
                    urls,
