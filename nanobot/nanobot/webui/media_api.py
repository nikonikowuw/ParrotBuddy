...
from nanobot.webui.file_preview import path_is_previewable
...
def strip_out_of_scope_paths(
    attachments: list[dict[str, Any]],
    *,
    scope: WorkspaceScope,
) -> None:
    """Drop the ``path`` field from attachments whose file lies outside *scope*.

    The WebUI opens file previews through workspace-scoped routes, so a
    ``path`` outside the scope would 403; worse, the presence of ``path``
    turns the attachment tile into a preview button that also drops the
    working download link. Omitting ``path`` keeps the signed media URL
    usable as a plain download for out-of-scope files. Mutates in place.
    """
    for att in attachments:
        raw = att.get("path")
        if raw and not path_is_previewable(raw, scope=scope):
            att.pop("path", None)


def strip_out_of_scope_media_urls(
    payload: dict[str, Any],
    *,
    scope: WorkspaceScope,
) -> None:
    """Strip ``path`` from every ``media_urls`` attachment in a session payload.

    Applies :func:`strip_out_of_scope_paths` to each message's
    ``media_urls`` list in ``payload["messages"]`` (the shape produced by
    :func:`attach_signed_media_urls` and the session/thread HTTP routes).
    """
    ...
    for msg in messages:
        if not isinstance(msg, dict):
            continue
        urls = msg.get("media_urls")
        if isinstance(urls, list):
            strip_out_of_scope_paths(urls, scope=scope)
...
def sign_or_stage_media_path(
    ...
    *,
    ...
    The returned dict carries the original ``path`` so WebUI clients can open
    the file in the preview panel (workspace-scoped) instead of only
    downloading the staged media copy.
    """
    signed = sign_media_path(path, secret=secret, media_dir=media_dir)
    if signed is not None:
        return {"url": signed, "name": path.name, "path": str(path)}
    try:
        if not path.is_file():
            return None
...
def signed_media_attachments(
    ...
    *,
    ...
    for pstr in paths:
        ...
        out.append({"kind": media_attachment_kind(name), "url": url, "name": name, "path": str(path)})
    return out
    ...
    *,
    ...
    for msg in messages:
        ...
        for entry in media:
            ...
            if signed is None:
                continue
            urls.append({"url": signed, "name": Path(entry).name, "path": str(Path(entry))})
        if urls:
            msg["media_urls"] = urls
        msg.pop("media", None)
