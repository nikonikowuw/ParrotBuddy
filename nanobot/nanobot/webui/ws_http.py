...
class GatewayHTTPHandler:
    ...
    async def _dispatch_session_routes(self, request: WsRequest, got: str) -> Response | None:
        ...
        m = re.match(r"^/api/sessions/([^/]+)/file$", got)
        if m:
            return await self._handle_file(request, m.group(1))
    ...
    def _sessions_list_payload(self) -> dict[str, Any]:
        ...
        for s in sessions:
            ...
            scope, lightrag_workspaces = self.workspaces.scope_and_lightrag_for_session_key(key)
            row["workspace_scope"] = scope.payload()
            row["lightrag_workspaces"] = lightrag_workspaces
            cleaned.append(row)
    ...
    def _handle_session_messages(self, request: WsRequest, key: str) -> Response:
        ...
        strip_out_of_scope_media_urls(data, scope=scope)
    ...
    def _handle_webui_thread_get(self, request: WsRequest, key: str) -> Response:
        ...
        if not _is_websocket_channel_session_key(decoded_key):
            return _http_error(404, "session not found")
        scope, lightrag_workspaces = self.workspaces.scope_and_lightrag_for_session_key(decoded_key)
        ...
        def _scoped_media(paths: list[str]) -> list[dict[str, Any]]:
            attachments = self.media.augment_transcript_media(paths)
            strip_out_of_scope_paths(attachments, scope=scope)
            return attachments

        data = build_webui_thread_response(
            decoded_key,
            augment_user_media=_scoped_media,
            augment_assistant_media=_scoped_media,
        ...
        data["lightrag_workspaces"] = lightrag_workspaces
        return _http_json_response(data)
    ...
    def _file_preview_blocking(
        self, decoded_key: str, path: str | None
    ) -> dict[str, Any]:
        scope = self.workspaces.scope_for_session_key(decoded_key)
        return file_preview_payload(path, scope=scope)

    async def _handle_file(self, request: WsRequest, key: str) -> Response:
        # Accepted tradeoff: the API token is also accepted as a query parameter
        # because <img>/<iframe>/<video> src URLs cannot send an Authorization
        # header. Exposure is bounded by the token TTL, and the response is
        # hardened with X-Content-Type-Options: nosniff, CSP sandbox for
        # script-capable types, and no-referrer media in the client.
        if not self.check_api_token(request):
            return _http_error(401, "Unauthorized")
        decoded_key = _decode_api_key(key)
        if decoded_key is None:
        ...
        # Script-capable types are sandboxed so a generated HTML/SVG file can
        # never read the WebUI token (also when opened directly in a new tab).
        if mime in {"text/html", "image/svg+xml"}:
            extra_headers.append(("Content-Security-Policy", "sandbox"))
        return _http_response(
            content,
            content_type=mime,
