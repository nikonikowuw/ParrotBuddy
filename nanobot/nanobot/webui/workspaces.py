...
# Session-metadata key for the WebUI-selected LightRAG knowledge-base set
# (per-chat). Mirrors ``WORKSPACE_SCOPE_METADATA_KEY`` for the KB selector.
LIGHTRAG_WORKSPACES_METADATA_KEY = "lightrag_workspaces"


def normalize_lightrag_workspaces(raw: Any) -> list[str]:
    """Normalize a raw ``lightrag_workspaces`` value into trimmed names.

    Accepts ``None`` (→ ``[]``) and sequences of scalar items; non-scalar
    entries and empty strings are dropped. Shared by the WebSocket channel and
    this controller so every parsing site agrees on the accepted shapes.
    """
...
class WebUIWorkspaceController:
    ...
    def _read_session_metadata(self, session_key: str) -> dict[str, Any]:
        """Read the session metadata dict (``{}`` when unavailable)."""
    ...
    def _scope_from_metadata(self, metadata: dict[str, Any]) -> WorkspaceScope:
        if WORKSPACE_SCOPE_METADATA_KEY not in metadata:
    ...
    def scope_for_session_key(self, session_key: str) -> WorkspaceScope:
        return self._scope_from_metadata(self._read_session_metadata(session_key))
    ...
    def persist_lightrag_workspaces(self, chat_id: str, workspaces: list[str]) -> None:
        """Persist the WebUI-selected LightRAG KB set for this chat (per-session)."""
        if self._sessions is None:
            return
        session = self._sessions.get_or_create(f"websocket:{chat_id}")
        session.metadata["webui"] = True
        session.metadata[LIGHTRAG_WORKSPACES_METADATA_KEY] = list(workspaces)
        self._sessions.save(session)

    def persist_message_metadata(
        self,
        chat_id: str,
        *,
        scope: WorkspaceScope,
        lightrag_workspaces: list[str] | None = None,
    ) -> None:
        """Persist per-chat WebUI metadata for an incoming message in one save.

        Combines the workspace scope and (optional) LightRAG KB selection into a
        single session save; ``lightrag_workspaces=None`` leaves the existing
        KB selection untouched.
        """
        if self._sessions is None:
            return
        session = self._sessions.get_or_create(f"websocket:{chat_id}")
        session.metadata["webui"] = True
        session.metadata[WORKSPACE_SCOPE_METADATA_KEY] = scope.metadata()
        if lightrag_workspaces is not None:
            session.metadata[LIGHTRAG_WORKSPACES_METADATA_KEY] = list(lightrag_workspaces)
        self._sessions.save(session)

    def _lightrag_from_metadata(self, metadata: dict[str, Any]) -> list[str]:
        return normalize_lightrag_workspaces(metadata.get(LIGHTRAG_WORKSPACES_METADATA_KEY))

    def lightrag_workspaces_for_session_key(self, session_key: str) -> list[str]:
        """Read the persisted LightRAG KB selection for a session (``[]`` if unset)."""
        return self._lightrag_from_metadata(self._read_session_metadata(session_key))

    def scope_and_lightrag_for_session_key(
        self, session_key: str
    ) -> tuple[WorkspaceScope, list[str]]:
        """Read scope and LightRAG KB selection with a single metadata read."""
        metadata = self._read_session_metadata(session_key)
        return self._scope_from_metadata(metadata), self._lightrag_from_metadata(metadata)
