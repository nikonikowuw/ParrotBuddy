...
# Reserved turn_id prefix for WebUI-internal system actions (e.g. a model
# preset hot-switch). Such turns are echoed live so the originating client
# can correlate them, but are never persisted to the transcript. Must match
# SYSTEM_COMMAND_TURN_PREFIX in webui/src/lib/nanobot-client.ts.
WEBUI_SYSTEM_TURN_PREFIX = "webui-system:"


def is_webui_system_turn_id(value: Any) -> bool:
    """Return True when *value* is a reserved WebUI system-action turn id."""
    return isinstance(value, str) and value.startswith(WEBUI_SYSTEM_TURN_PREFIX)
...
class WebUITranscriptRecorder:
    ...
    def prepare_and_append(
        ...
        *,
        ...
        if is_webui_system_turn_id((metadata or {}).get(WEBUI_TURN_METADATA_KEY)):
            # WebUI-internal system turn: annotate the live event (so the
            # originating client can correlate it) but never persist it, so it
            # cannot resurface in history replay. Guards every outbound event
            # kind (message, turn_end, ...) in one place.
            return
        record = dict(event)
    ...
    def append_user_message(
        ...
        *,
        ...
        if text.strip() == "/stop" and not media_paths:
            return
        if is_webui_system_turn_id(metadata.get(WEBUI_TURN_METADATA_KEY)):
            # WebUI-internal system action (e.g. model preset hot-switch): the
            # command still runs, but its user text is never persisted so it
            # cannot resurface as a chat bubble in history replay.
            return
