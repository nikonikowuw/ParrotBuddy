...
export async function listSessions(
  ...
  return body.sessions.map((s) => ({
    ...
    lightragWorkspaces: s.lightrag_workspaces ?? [],
  }));
}
...
/**
 * Absolute URL that serves the raw file bytes for the current session.
 *
 * Used as the ``src`` of media/iframe elements, which cannot send an
 * ``Authorization`` header, so the API token is passed as a query parameter
 * (the gateway accepts both forms).
 */
