...
export class NanobotClient {
  ...
  /**
   * Persist an edited file back to its original workspace path.
   *
   * The editor sends the full file content as a base64 string over the
   * WebSocket (the HTTP surface is GET-only). The server resolves the path
   * against the session's workspace scope and writes it atomically, then
   * replies with ``file_saved`` / ``file_save_error`` carrying the same
   * ``request_id``. Resolves with the saved absolute path.
   */
  saveFile(
    chatId: string,
    path: string,
  ...
  sendMessage(
    ...
    const frame: Outbound = {
      ...
      ...(options?.lightragWorkspaces?.length ? { lightrag_workspaces: options.lightragWorkspaces } : {}),
  ...
  /**
   * Send a slash command as an internal system action. The turn is tagged
   * with a reserved ``turn_id`` so the resulting events are swallowed instead
   * of rendered as chat messages; the returned promise resolves once the
   * command turn completes (or rejects on timeout / socket loss).
   */
  ...
  setLightragWorkspaces(chatId: string, lightragWorkspaces: string[]): void {
    this.knownChats.add(chatId);
    this.queueSend({
      type: "set_lightrag_workspaces",
      chat_id: chatId,
      lightrag_workspaces: lightragWorkspaces,
    });
  }
  ...
  private handleMessage(ev: MessageEvent): void {
    ...
    if (isSystemCommandTurnId(turnId)) {
      // Resolve on the first matching ``message`` OR ``turn_end``. ``message``
      // is required because standalone commands (e.g. ``/model`` on an idle
      // chat) are dispatched inline and never emit a ``turn_end`` — the reply
      // message is their completion signal. ``turn_end`` additionally covers
      // commands injected into a running agent turn.
    ...
    if (parsed.event === "file_saved") {
      this.resolveFileSave(parsed.request_id, parsed.path);
      return;
    }

    if (parsed.event === "file_save_error") {
  ...
  private emitSessionUpdate(
    ...
    for (const handler of this.sessionUpdateHandlers) {
      handler(chatId, scope, workspaceScope, lightragWorkspaces);
    }
  }
  ...
  private resolveFileSave(requestId: string | undefined, path: string | undefined): void {
    if (!requestId) return;
