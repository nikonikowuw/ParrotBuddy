...
function Shell({
  ...
  useEffect(() => {
    ...
    setKnowledgeBasesOverrides((current) => {
      const entries = Object.entries(current).filter(([chatId]) => knownChatIds.has(chatId));
      return entries.length === Object.keys(current).length ? current : Object.fromEntries(entries);
    });
  }, [loading, sessions]);
  ...
  useEffect(() => {
    return client.onSessionUpdate((chatId, scope, workspaceScope, lightragWorkspaces) => {
      if (scope === "thread") {
        setUpdatedChatIds((current) => {
          const next = new Set(current);
      ...
      if (lightragWorkspaces) {
        setKnowledgeBasesOverrides((current) => ({
          ...current,
          [chatId]: lightragWorkspaces,
        }));
        setDraftKnowledgeBases(lightragWorkspaces);
      }
    });
  ...
  const applyKnowledgeBases = useCallback(
    (bases: string[]) => {
      if (activeChatId) {
        // Instant local override so the composer reflects the change before the
        // server echoes `session_updated`; persists server-side immediately.
        setKnowledgeBasesOverrides((current) => ({ ...current, [activeChatId]: bases }));
        if (!activeChatRunning) {
          client.setLightragWorkspaces(activeChatId, bases);
        }
        return;
      }
      setDraftKnowledgeBases(bases);
    },
    [activeChatId, activeChatRunning, client],
  );
        ...
        >
            ...
            >
              <ThreadShell
                ...
                selectedKnowledgeBases={activeKnowledgeBases}
                onKnowledgeBasesChange={applyKnowledgeBases}
