...
export interface ModelPresetOption {
  ...
  /** Provider has no usable API key — show as disabled in the picker. */
  disabled?: boolean;
}
...
function ComposerModelBadge({
  ...
  const hasAction = Boolean(onClick) || Boolean(onPresetChange);
  const activePresetName = modelPreset?.trim() || "";
  const Container = hasAction ? "button" : "span";

  const badge = (
    <Container
      title={title}
      type={hasAction ? "button" : undefined}
      aria-label={hasAction ? `${label} (${activePresetName || "default"})` : undefined}
      aria-haspopup={hasAction ? "menu" : undefined}
      data-testid={hasAction ? "composer-model-switcher" : undefined}
      className={cn(
        "inline-flex min-w-0 items-center rounded-full border border-border/55 bg-card font-medium text-foreground/82",
        "shadow-[0_2px_8px_rgba(15,23,42,0.045)]",
        hasAction && "cursor-pointer hover:bg-accent/55 hover:text-foreground",
    ...
    >
      ...
      {hasAction ? <ChevronDown className="h-3 w-3 shrink-0 text-muted-foreground/70" aria-hidden /> : null}
    </Container>
  );

  if (!hasAction) return badge;
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent
        align="end"
        className="max-h-[20rem] w-[min(20rem,calc(100vw-2rem))] overflow-y-auto scrollbar-thin scrollbar-track-transparent"
      >
            ...
            return (
              <DropdownMenuItem
                ...
                className={cn(
                  ...
                  preset.disabled && "opacity-50",
                )}
              >
                <span className="min-w-0 truncate">
                  {preset.label || preset.name}
                  {preset.model ? (
                    <span className="ml-2 truncate text-muted-foreground">{preset.model}</span>
                  ) : null}
                </span>
                {selected ? <Check className="h-3.5 w-3.5 shrink-0 text-foreground" aria-hidden /> : null}
              </DropdownMenuItem>
            );
          })
        ) : (
          <div className="px-2.5 py-2 text-[12.5px] leading-5 text-muted-foreground">
            {t("thread.composer.noConfiguredModels", {
              defaultValue: "No model configurations yet — add one to switch models here.",
            })}
          </div>
        )}
        {onClick ? (
          <>
            ...
            <DropdownMenuItem
              onSelect={onClick}
              className="flex cursor-default items-center gap-2 rounded-[12px] px-2.5 py-2 text-[13px] font-medium text-foreground focus:bg-muted/85"
            >
