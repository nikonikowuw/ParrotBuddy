...
const FILE_PREVIEW_DEFAULT_WIDTH = 720;
const FILE_PREVIEW_MIN_WIDTH = 360;
// Upper bound for manual resizing. Auto-fit may request wider (an A4 page
// rendered at 100% by docx-preview needs ~854px, and wide tables can need
// more), so auto-fit uses its own, looser cap below.
const FILE_PREVIEW_MAX_WIDTH = 860;
const FILE_PREVIEW_MIN_MAIN_WIDTH = 420;
// When auto-fitting to a document's natural width, allow the chat thread to
...
function maxAutoFitFilePreviewWidth(containerWidth: number): number {
  return Math.max(
    FILE_PREVIEW_MIN_WIDTH,
    Math.min(FILE_PREVIEW_MAX_WIDTH + 240, containerWidth - FILE_PREVIEW_MIN_MAIN_WIDTH_AUTOFIT),
  );
}
...
function activeModelPreset(
  ...
  presetNameOverride: string | null = null,
): SettingsPayload["model_presets"][number] | null {
  if (!settings) return null;
  const configured = presetNameOverride || settings.agent.model_preset || "default";
  return (
...
function resolvedModelProvider(
  ...
  const preset = activeModelPreset(settings, presetNameOverride);
...
function toModelBadgeInfo(
  ...
  const preset = activeModelPreset(settings, presetNameOverride);
  // Prefer the selected preset's model so the badge reflects a switch
  // immediately, even before the runtime model broadcast arrives.
  const model = preset?.model || modelName || settings?.agent.model || null;
  const label = toModelBadgeLabel(model);
  const provider = resolvedModelProvider(settings, model, presetNameOverride);
...
function modelPresetOptionsFromSettings(settings: SettingsPayload | null): ModelPresetOption[] {
  if (!settings) return [];
  return settings.model_presets.map((preset) => {
    const provider =
    ...
    return {
      ...
      disabled: !providerRow?.configured,
    };
  });
}
...
export function ThreadShell({
  ...
  const activeModelPresetName = localModelPreset || settings?.agent.model_preset || "default";
  const handleModelPresetChange = useCallback((name: string) => {
    // Optimistic UI update first so the badge reflects the choice instantly.
    setLocalModelPreset(name);
    // Persist the selection so it survives restarts and applies to future turns
    // (the agent loop refreshes the active preset from config on each turn).
    void updateSettings(token, { modelPreset: name })
      .then((updated) => setSettings(updated))
      .catch(() => {
        // Keep the optimistic state; next settings refresh reconciles.
      });
    // Hot-switch the running loop immediately when a chat is open.
    if (chatId) {
  ...
  const modelBadge = useMemo(
    () => toModelBadgeInfo(modelName, settings, activeModelPresetName),
    [activeModelPresetName, modelName, settings],
  );
  ...
  const withLightragWorkspaces = useCallback(
    (options?: SendOptions): SendOptions | undefined => {
      if (!selectedKnowledgeBases.length) return options;
      return {
        ...(options ?? {}),
        lightragWorkspaces: selectedKnowledgeBases,
      };
    },
    [selectedKnowledgeBases],
  );
  ...
  const handleThreadSend = useCallback(
    (content: string, images?: SendImage[], options?: SendOptions) => {
      setScrollToLatestUserPromptSignal((value) => value + 1);
      send(content, images, withLightragWorkspaces(withWorkspaceScope(options)));
    },
    [send, withLightragWorkspaces, withWorkspaceScope],
  );
  ...
  const handleAutoFitFilePreviewWidth = useCallback((contentWidth: number) => {
    // Expand (never shrink) the panel so the rendered document's full
    // width fits — docx-preview lays out pages at a fixed 100% scale.
    const shellWidth = shellRef.current?.getBoundingClientRect().width ?? window.innerWidth;
    const maxWidth = maxAutoFitFilePreviewWidth(shellWidth);
    const nextWidth = clampFilePreviewWidth(
      Math.max(filePreviewWidthRef.current, contentWidth),
      maxWidth,
    );
    ...
    <>
      ...
      {session ? (
        <ThreadComposer
          ...
          modelPreset={activeModelPresetName}
    ...
    <section ref={shellRef} className="relative flex min-h-0 flex-1 overflow-hidden">
      ...
      {filePreviewPath && historyKey ? (
        <FilePreviewPanel
          ...
          onAutoFitWidth={handleAutoFitFilePreviewWidth}
          onSaveFile={
            chatId && client
              ? async (savePath, contentBase64) => {
                await client.saveFile(chatId, savePath, contentBase64);
              }
              : undefined
          }
        />
