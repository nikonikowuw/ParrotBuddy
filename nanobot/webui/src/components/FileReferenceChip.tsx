...
export function FileReferenceIcon({ kind }: { kind: FileReferenceKind }) {
  ...
  if (kind === "word" || kind === "excel" || kind === "powerpoint" || kind === "pdf") {
    ...
    // Office file icon look. Filled with each app's brand color.
    const officeIcons = {
      word: { Icon: FaFileWord, color: "#2B579A" },
      excel: { Icon: FaFileExcel, color: "#217346" },
      powerpoint: { Icon: FaFilePowerpoint, color: "#B7472A" },
      pdf: { Icon: FaFilePdf, color: "#D93025" },
    } as const;
    const { Icon, color } = officeIcons[kind];
    return (
      <span
        className="inline-flex h-[1em] shrink-0 items-center justify-center"
        aria-hidden
      >
        <Icon className="h-full w-auto" color={color} />
      </span>
    );
  }
