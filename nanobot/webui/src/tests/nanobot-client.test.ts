...
describe("NanobotClient", () => {
  ...
  it("dispatches lightrag_workspaces in session updates", () => {
    const client = new NanobotClient({
      url: "ws://test",
      reconnect: false,
      socketFactory: (url) => new FakeSocket(url) as unknown as WebSocket,
    ...
    lastSocket().fakeMessage({
      ...
      chat_id: "chat-kb",
      scope: "metadata",
      lightrag_workspaces: ["notes", "code"],
    });

    expect(handler).toHaveBeenCalledWith("chat-kb", "metadata", undefined, ["notes", "code"]);
  });

  it("sends a set_lightrag_workspaces frame", () => {
    const client = new NanobotClient({
      url: "ws://test",
      reconnect: false,
      socketFactory: (url) => new FakeSocket(url) as unknown as WebSocket,
    ...
    client.connect();
    lastSocket().fakeOpen();
    client.setLightragWorkspaces("chat-1", ["a", "b"]);

    expect(lastSocket().sent.at(-1)).toEqual(
      JSON.stringify({
        type: "set_lightrag_workspaces",
        chat_id: "chat-1",
        lightrag_workspaces: ["a", "b"],
      }),
    );
  });
  ...
  it("sends save_file requests and resolves file_saved outside chat dispatch", async () => {
    const client = new NanobotClient({
      url: "ws://test",
      reconnect: false,
      socketFactory: (url) => new FakeSocket(url) as unknown as WebSocket,
    ...
    const promise = client.saveFile("chat-a", "/Users/hr/workspace/sales.xlsx", "QkFTRTY0", 1_000);
    const frame = JSON.parse(lastSocket().sent.at(-1) as string);
    expect(frame).toMatchObject({
      type: "save_file",
      chat_id: "chat-a",
      path: "/Users/hr/workspace/sales.xlsx",
      content_b64: "QkFTRTY0",
    });
    expect(typeof frame.request_id).toBe("string");
    ...
    await expect(promise).resolves.toBe("/Users/hr/workspace/sales.xlsx");
    // save results are transport-level, not dispatched as a chat event
    expect(handler).not.toHaveBeenCalled();
  });

  it("rejects pending save_file requests on errors and socket close", async () => {
    const client = new NanobotClient({
      url: "ws://test",
      reconnect: false,
      socketFactory: (url) => new FakeSocket(url) as unknown as WebSocket,
    ...
    const errored = client.saveFile("chat-a", "/x.xlsx", "QUFB", 1_000);
    const errorFrame = JSON.parse(lastSocket().sent.at(-1) as string);
    lastSocket().fakeMessage({
    ...
    const dropped = client.saveFile("chat-a", "/y.xlsx", "QkJC", 1_000);
    lastSocket().close();
    await expect(dropped).rejects.toThrow("socket closed");
  });
  ...
  it("sends system commands under a reserved turn id and resolves on turn completion", async () => {
    const client = new NanobotClient({
      url: "ws://test",
      reconnect: false,
      socketFactory: (url) => new FakeSocket(url) as unknown as WebSocket,
    ...
    const chatHandler = vi.fn();
    client.onChat("chat-sys", chatHandler);
    client.connect();
    lastSocket().fakeOpen();

    const promise = client.sendSystemCommand("chat-sys", "/model fast");
    const frame = JSON.parse(lastSocket().sent.at(-1) as string) as {
      type: string;
      chat_id: string;
      content: string;
      turn_id: string;
    ...
    // System-command events are swallowed (not dispatched to chat handlers).
    lastSocket().fakeMessage({
      event: "message",
      chat_id: "chat-sys",
  ...
  it("rejects system commands on socket close", async () => {
    const client = new NanobotClient({
      url: "ws://test",
      reconnect: false,
      socketFactory: (url) => new FakeSocket(url) as unknown as WebSocket,
  ...
  it("rejects system commands after a timeout", async () => {
    const client = new NanobotClient({
      url: "ws://test",
      reconnect: false,
      socketFactory: (url) => new FakeSocket(url) as unknown as WebSocket,
    ...
    const promise = client.sendSystemCommand("chat-sys", "/model fast", 100);
    const assertion = expect(promise).rejects.toThrow("system command timed out");
