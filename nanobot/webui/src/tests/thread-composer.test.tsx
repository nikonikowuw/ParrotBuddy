...
describe("ThreadComposer", () => {
  ...
  it("switches model presets from the composer badge dropdown", async () => {
    ...
    render(
      <ThreadComposer
        ...
        modelPreset="default"
        modelPresets={[
          { name: "default", label: "Default", model: "gpt-4o", provider: "openai" },
          { name: "fast", label: "Fast writing", model: "gpt-4.1-mini", provider: "openai" },
        ]}
    ...
    const switcher = screen.getByTestId("composer-model-switcher");
    expect(switcher).toBeInTheDocument();
    fireEvent.pointerDown(switcher);

    const option = await screen.findByText("Fast writing");
    expect(option).toBeInTheDocument();
    fireEvent.click(option);
    expect(onModelPresetChange).toHaveBeenCalledWith("fast");
    expect(onSend).not.toHaveBeenCalled();
  });

  it("does not open a model switcher when no presets are available", () => {
    render(
      <ThreadComposer
        onSend={vi.fn()}
        modelLabel="gpt-4o"
    ...
    expect(screen.queryByTestId("composer-model-switcher")).not.toBeInTheDocument();
  });
