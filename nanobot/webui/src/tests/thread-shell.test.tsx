...
function makeClient() {
  ...
  return {
    ...
    sendMessage: vi.fn(),
    sendSystemCommand: vi.fn(() => Promise.resolve()),
    newChat: vi.fn(),
    forkChat: vi.fn(),
    attach: vi.fn(),
    connect: vi.fn(),
...
describe("ThreadShell", () => {
  ...
  it("opens model settings from the unconfigured model badge", async () => {
    ...
    const badge = await screen.findByTestId("composer-model-switcher");
    ...
    // The badge opens the picker; unconfigured providers are shown disabled
    // and the bottom action routes to model settings.
    fireEvent.pointerDown(badge);
    const unconfiguredOption = await screen.findByRole("menuitem", { name: /openai-codex\/gpt-5\.1-codex/ });
    expect(unconfiguredOption).toHaveAttribute("aria-disabled", "true");
    fireEvent.click(screen.getByRole("menuitem", { name: "Configure model" }));
  ...
  it("switches the model preset from the composer badge", async () => {
    const client = makeClient();
    ...
    const fetchMock = vi.fn(async (input: RequestInfo | URL) => {
      ...
      if (url.includes("/api/settings/update")) {
        return httpJson({
          ...settings,
          agent: { ...settings.agent, model_preset: "fast" },
        });
      }
    ...
    const switcher = await screen.findByTestId("composer-model-switcher");
    fireEvent.pointerDown(switcher);
    fireEvent.click(await screen.findByText("Fast writing"));

    expect(fetchMock).toHaveBeenCalledWith(
      expect.stringContaining("/api/settings/update?model_preset=fast"),
      expect.anything(),
    );
    expect(client.sendSystemCommand).toHaveBeenCalledWith("model-switch", "/model fast");
    expect(client.sendMessage).not.toHaveBeenCalled();
  });

  it("switches the model preset from the welcome composer when no chat is open", async () => {
    const client = makeClient();
    const settings = modelSettings("openai-codex/gpt-5.5", "openai_codex");
    settings.agent.model_preset = "default";
    settings.model_presets = [
  ...
  it("opens model settings from the welcome composer badge when no chat is open", async () => {
    const client = makeClient();
    const settings = modelSettings("openai-codex/gpt-5.5", "openai_codex");
    const onOpenModelSettings = vi.fn();
