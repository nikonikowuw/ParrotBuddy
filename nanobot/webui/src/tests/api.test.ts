    ...
    await expect(listSessions("tok")).resolves.toMatchObject([
      {
        ...
        lightragWorkspaces: ["notes", "code"],
      },
    ]);
  });

  it("defaults lightragWorkspaces to [] when the server omits it", async () => {
    vi.mocked(fetch).mockResolvedValueOnce({
      ok: true,
      json: async () => ({
        sessions: [
          {
            key: "websocket:chat-no-kb",
            created_at: "2026-05-01T10:00:00",
            updated_at: "2026-05-01T10:01:00",
          },
        ],
    ...
    expect(rows[0]?.lightragWorkspaces).toEqual([]);
  });
