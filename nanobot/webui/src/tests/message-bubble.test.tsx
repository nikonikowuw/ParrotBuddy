...
describe("MessageBubble", () => {
  ...
  it("opens the preview panel when clicking a file attachment that carries a path", () => {
    ...
    const message: UIMessage = {
      ...
      media: [
        {
          kind: "file",
          url: "/api/media/sig/docx",
          name: "AI_Agent_介绍.docx",
          path: "/Users/zhang/.nanobot/workspace/AI_Agent_介绍.docx",
        },
      ],
    };
    ...
    const tile = screen.getByLabelText("File attachment");
    expect(tile.tagName).toBe("BUTTON");
    expect(container.querySelector('a[download]')).not.toBeInTheDocument();
    // Office files get a branded kind icon instead of the generic file glyph.
    expect(tile.querySelector("svg")?.getAttribute("color")).toBe("#2B579A");

    fireEvent.click(tile);
    expect(onOpenFilePreview).toHaveBeenCalledWith(
      "/Users/zhang/.nanobot/workspace/AI_Agent_介绍.docx",
    );
  });

  it("keeps a download link for file attachments without a workspace path", () => {
    const onOpenFilePreview = vi.fn();
    const message: UIMessage = {
      id: "a-orphan",
      role: "assistant",
    ...
    expect(link).toHaveAttribute("href", "/api/media/sig/legacy");
