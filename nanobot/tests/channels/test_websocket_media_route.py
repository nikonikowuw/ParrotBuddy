...
def test_sign_or_stage_media_path_carries_original_path(tmp_path) -> None:
    from nanobot.webui.media_gateway import WebUIMediaGateway

    media = tmp_path / "media"
    media.mkdir()
    source = tmp_path / "report.docx"
    source.write_bytes(b"PK\x03\x04\x00fake-docx")

    gateway = WebUIMediaGateway(workspace_path=tmp_path, logger=None)
    with patch("nanobot.webui.media_gateway.get_media_dir", return_value=media):
        att = gateway.sign_or_stage_media_path(source)

    assert att is not None
    assert att["name"] == "report.docx"
    assert att["path"] == str(source)
    assert att["url"].startswith("/api/media/")
    # the out-of-media-root file was staged inside the media root
    assert any(media.glob("*-report.docx"))


def test_signed_media_attachments_carry_original_path(tmp_path) -> None:
    from nanobot.webui.media_api import signed_media_attachments
    from nanobot.webui.media_gateway import WebUIMediaGateway

    media = tmp_path / "media"
    media.mkdir()
    source = tmp_path / "data" / "deck.pptx"
    source.parent.mkdir()
    source.write_bytes(b"PK\x03\x04\x00fake-pptx")

    gateway = WebUIMediaGateway(workspace_path=tmp_path, logger=None)
    with patch("nanobot.webui.media_gateway.get_media_dir", return_value=media):
        out = signed_media_attachments(
            [str(source)],
            sign_path=gateway.sign_or_stage_media_path,
        )

    assert len(out) == 1
    assert out[0]["kind"] == "file"
    assert out[0]["name"] == "deck.pptx"
    assert out[0]["path"] == str(source)
    assert out[0]["url"].startswith("/api/media/")


def test_attach_signed_media_urls_carries_original_path(tmp_path) -> None:
    from nanobot.webui.media_api import attach_signed_media_urls
    from nanobot.webui.media_gateway import WebUIMediaGateway

    media = tmp_path / "media"
    media.mkdir()
    source = media / "notes.docx"
    source.write_bytes(b"PK\x03\x04fake-docx")

    gateway = WebUIMediaGateway(workspace_path=tmp_path, logger=None)
    payload = {"messages": [{"role": "assistant", "text": "ok", "media": [str(source)]}]}
    with patch("nanobot.webui.media_gateway.get_media_dir", return_value=media):
        attach_signed_media_urls(payload, sign_path=gateway.sign_media_path)

    urls = payload["messages"][0]["media_urls"]
    assert len(urls) == 1
    assert urls[0]["url"].startswith("/api/media/")
    assert urls[0]["name"] == "notes.docx"
    assert urls[0]["path"] == str(source)
    assert "media" not in payload["messages"][0]


def _media_route_scope(project_path: Path, *, restrict: bool) -> WorkspaceScope:
...
def test_strip_out_of_scope_paths_keeps_in_scope_drops_out_of_scope(tmp_path) -> None:
    from nanobot.webui.media_api import strip_out_of_scope_paths

    workspace = tmp_path / "workspace"
    workspace.mkdir()
    in_scope = workspace / "report.xlsx"
    in_scope.write_bytes(b"PK")
    media = tmp_path / "media"
    media.mkdir()
    out_of_scope = media / "upload.pdf"
    out_of_scope.write_bytes(b"%PDF")

    scope = _media_route_scope(workspace, restrict=True)
    attachments = [
        {"url": "/api/media/1", "name": "report.xlsx", "path": str(in_scope)},
        {"url": "/api/media/2", "name": "upload.pdf", "path": str(out_of_scope)},
    ]
    strip_out_of_scope_paths(attachments, scope=scope)

    assert attachments[0]["path"] == str(in_scope)
    # out-of-scope attachments keep their signed URL (download) but lose the
    # preview path so the client cannot open a 403 preview or drop the link.
    assert "path" not in attachments[1]
    assert attachments[1]["url"] == "/api/media/2"


def test_strip_out_of_scope_paths_full_access_keeps_everything(tmp_path) -> None:
    from nanobot.webui.media_api import strip_out_of_scope_paths
    ...
    scope = _media_route_scope(workspace, restrict=False)
    attachments = [
        {"url": "/api/media/1", "name": "upload.pdf", "path": str(out_of_scope)},
    ]
    strip_out_of_scope_paths(attachments, scope=scope)

    # Full access previews any existing file, so the path survives.
    assert attachments[0]["path"] == str(out_of_scope)


def test_strip_out_of_scope_media_urls_scopes_session_payload(tmp_path) -> None:
    from nanobot.webui.media_api import strip_out_of_scope_media_urls
    ...
    strip_out_of_scope_media_urls(payload, scope=scope)

    urls = payload["messages"][0]["media_urls"]
    assert urls[0]["path"] == str(in_scope)
    assert "path" not in urls[1]
