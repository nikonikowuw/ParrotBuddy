...
@pytest.mark.asyncio
async def test_webui_system_turn_message_is_not_persisted(
    bus: MagicMock,
    tmp_path,
    monkeypatch,
) -> None:
    ...
    async def answer_during_publish(_msg: Any) -> None:
        # The command reply carries the same system turn id in metadata (as
        # cmd_model does via {**dict(ctx.msg.metadata), ...}).
        await channel.send(
            OutboundMessage(
                channel="websocket",
                chat_id="chat-1",
                content="Switched model preset to `fast`.",
                metadata={"webui_turn_id": "webui-system:abc123"},
            )
        )
    ...
    await channel._dispatch_envelope(
        ...
        "webui-client",
        {
            ...
            "content": "/model fast",
            "webui": True,
            "turn_id": "webui-system:abc123",
        },
    )

    msg = bus.publish_inbound.await_args.args[0]
    assert msg.content == "/model fast"
    # Neither the user bubble nor the command reply is persisted, so the
    # system turn cannot resurface as chat messages in history replay.
    assert read_transcript_lines("websocket:chat-1") == []
...
@pytest.mark.asyncio
async def test_webui_set_lightrag_workspaces_persists_and_echoes(
    ...
    await channel._dispatch_envelope(
        ...
        "webui-client",
        {
            "type": "set_lightrag_workspaces",
            "chat_id": "chat-kb",
            "lightrag_workspaces": ["notes", 7, "  code  ", "", None],
        },
    )

    payload = json.loads(conn.send.await_args.args[0])
    assert payload["event"] == "session_updated"
    assert payload["chat_id"] == "chat-kb"
    # Strings are trimmed, numeric entries are kept, blanks/None dropped.
    assert payload["lightrag_workspaces"] == ["notes", "7", "code"]

    saved = sessions.read_session_file("websocket:chat-kb")
    assert saved["metadata"]["lightrag_workspaces"] == ["notes", "7", "code"]
    bus.publish_inbound.assert_not_awaited()


@pytest.mark.asyncio
async def test_webui_set_lightrag_workspaces_rejects_invalid_chat_id(
    bus: MagicMock, tmp_path
) -> None:
    default_workspace = tmp_path / "default"
    default_workspace.mkdir()
    ...
    await channel._dispatch_envelope(
        ...
        "webui-client",
        {
            "type": "set_lightrag_workspaces",
            "chat_id": "../evil/chat",
            "lightrag_workspaces": ["notes"],
        },
    )

    payload = json.loads(conn.send.await_args.args[0])
    assert payload["event"] == "error"
    assert payload["detail"] == "invalid chat_id"
    bus.publish_inbound.assert_not_awaited()


@pytest.mark.asyncio
async def test_webui_message_without_lightrag_field_does_not_wipe_selection(
    bus: MagicMock, tmp_path
) -> None:
    default_workspace = tmp_path / "default"
    default_workspace.mkdir()
    ...
    conn = AsyncMock()
    conn.remote_address = ("127.0.0.1", 50123)
    channel._workspaces.persist_lightrag_workspaces("chat-kb", ["notes"])

    # A message that does not carry the field (e.g. from a non-WebUI client)
    # must not wipe the persisted per-chat KB selection.
    await channel._dispatch_envelope(
        conn,
        "webui-client",
        {"type": "message", "chat_id": "chat-kb", "content": "hi"},
    )

    saved = sessions.read_session_file("websocket:chat-kb")
    assert saved["metadata"]["lightrag_workspaces"] == ["notes"]


@pytest.mark.asyncio
async def test_webui_message_with_lightrag_field_persists_selection(
    bus: MagicMock, tmp_path
) -> None:
    default_workspace = tmp_path / "default"
    default_workspace.mkdir()
    ...
    await channel._dispatch_envelope(
        ...
        "webui-client",
        {
            ...
            "content": "hi",
            "webui": True,
            "lightrag_workspaces": ["code"],
        },
    )

    saved = sessions.read_session_file("websocket:chat-kb")
    assert saved["metadata"]["lightrag_workspaces"] == ["code"]
...
async def test_settings_api_returns_safe_subset_and_updates_whitelist(
    ...
    try:
        ...
        assert workspace_path.endswith("/.pipi/workspace")
...
def test_sessions_list_includes_active_run_started_at(monkeypatch) -> None:
    ...
    lightrag_workspaces = body["sessions"][0].pop("lightrag_workspaces")
    assert lightrag_workspaces == []
...
async def test_handle_file_serves_workspace_document_inline(tmp_path) -> None:
    from urllib.parse import quote
    ...
    source.parent.mkdir(parents=True)
    payload = b"PK\x03\x04\x00binary-content"
    ...
    enc = quote(key, safe="")
    req = Request(
        f"/api/sessions/{enc}/file?path={quote('report.docx', safe='')}",
        Headers([("Authorization", "Bearer tok")]),
    )
    ...
    assert "filename*=UTF-8''report.docx" in resp.headers.get("Content-Disposition")
    assert resp.headers.get("X-Content-Type-Options") == "nosniff"
    assert resp.headers.get("Content-Security-Policy") is None


async def test_handle_file_sandboxes_scriptable_types(tmp_path) -> None:
    from urllib.parse import quote
    ...
    workspace = tmp_path / "workspace"
    workspace.mkdir()
    (workspace / "page.html").write_text("<script>alert(1)</script>\n", encoding="utf-8")
    (workspace / "logo.svg").write_text("<svg xmlns='http://www.w3.org/2000/svg'/>", encoding="utf-8")
    ...
    for name in ("page.html", "logo.svg"):
        req = Request(
            f"/api/sessions/{enc}/file?path={quote(name, safe='')}",
            Headers([("Authorization", "Bearer tok")]),
        )
        resp = await gateway.http._handle_file(req, enc)
        assert resp.status_code == 200
...
async def test_handle_file_rejects_paths_outside_workspace(tmp_path) -> None:
    from urllib.parse import quote
    ...
    enc = quote(key, safe="")
    req = Request(
        f"/api/sessions/{enc}/file?path={quote(str(outside), safe='')}",
        Headers([("Authorization", "Bearer tok")]),
    )
...
async def test_handle_file_rejects_oversized_file(tmp_path) -> None:
    from urllib.parse import quote

    from websockets.datastructures import Headers
    from websockets.http11 import Request

    from nanobot.webui import file_preview as fp

    workspace = tmp_path / "workspace"
    workspace.mkdir()
    (workspace / "huge.bin").write_bytes(b"x" * (fp.MAX_FILE_SERVE_BYTES + 1))
    ...
    enc = quote(key, safe="")
    req = Request(
        f"/api/sessions/{enc}/file?path={quote('huge.bin', safe='')}",
        Headers([("Authorization", "Bearer tok")]),
    )
...
async def test_file_preview_reports_binary_kind_for_documents(tmp_path) -> None:
    from urllib.parse import quote
    ...
    source = workspace / "report.docx"
    source.parent.mkdir(parents=True)
    source.write_bytes(b"PK\x03\x04\x00" + b"\x00" * 64)
    ...
    enc = quote(key, safe="")
    req = Request(
        f"/api/sessions/{enc}/file-preview?path={quote('report.docx', safe='')}",
        Headers([("Authorization", "Bearer tok")]),
    )
