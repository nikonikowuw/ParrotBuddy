...
def _kb_controller(tmp_path, monkeypatch):
    monkeypatch.setattr("nanobot.webui.workspaces.get_webui_dir", lambda: tmp_path / "webui")
    default = tmp_path / "default"
    default.mkdir()
    sessions = SessionManager(tmp_path / "sessions")
...
def test_persist_lightrag_workspaces_round_trip(tmp_path, monkeypatch) -> None:
    controller = _kb_controller(tmp_path, monkeypatch)

    assert controller.lightrag_workspaces_for_session_key("websocket:kb-chat") == []

    controller.persist_lightrag_workspaces("kb-chat", ["notes", "code"])
    assert controller.lightrag_workspaces_for_session_key("websocket:kb-chat") == [
        "notes",
        "code",
    ]
...
def test_persist_lightrag_workspaces_clears_with_empty_list(tmp_path, monkeypatch) -> None:
    controller = _kb_controller(tmp_path, monkeypatch)
    controller.persist_lightrag_workspaces("kb-chat", ["a"])
    assert controller.lightrag_workspaces_for_session_key("websocket:kb-chat") == ["a"]

    controller.persist_lightrag_workspaces("kb-chat", [])
    assert controller.lightrag_workspaces_for_session_key("websocket:kb-chat") == []


def test_lightrag_workspaces_for_session_key_without_session_manager(tmp_path, monkeypatch) -> None:
    monkeypatch.setattr("nanobot.webui.workspaces.get_webui_dir", lambda: tmp_path / "webui")
    default = tmp_path / "default"
    default.mkdir()
    controller = WebUIWorkspaceController(
    ...
    assert controller.lightrag_workspaces_for_session_key("websocket:any") == []


def test_lightrag_workspaces_for_session_key_reads_metadata_only(tmp_path, monkeypatch) -> None:
    controller = _kb_controller(tmp_path, monkeypatch)
    controller.persist_lightrag_workspaces("metadata-only", ["a", "b"])

    def fail_full_read(_key: str) -> None:
        raise AssertionError("KB lookup should not read full session history")

    monkeypatch.setattr(
        controller._sessions, "read_session_file", fail_full_read, raising=False
    )

    assert controller.lightrag_workspaces_for_session_key("websocket:metadata-only") == [
        "a",
        "b",
    ]
